...
{
	FunctionName = FName("RoutingNotMatched");
	FunctionClass = UUnrealMosquittoRoutingLibrary::StaticClass();
	// Set properties on the function pin
	const UEdGraphSchema_K2 *K2Schema = GetDefault<UEdGraphSchema_K2>();
	UEdGraphPin *FunctionPin = CreatePin(EGPD_Input, K2Schema->PC_Object, FunctionClass, FunctionName, FCreatePinParams());
...
{
	FName PinName = FName("/richard");
	Routes.Add(PinName);

	const UEdGraphSchema_K2 *K2Schema = GetDefault<UEdGraphSchema_K2>();
	CreatePin(EGPD_Output, K2Schema->PC_Exec, PinName, FCreatePinParams());
	/*FunctionClass = UUnrealMosquittoRoutingLibrary::StaticClass();
	FunctionName = TEXT("RoutingNotMatched");*/
}
...
void UUnrealMosquittoRouter::CreateSelectionPin()
{
	...
	const UEdGraphSchema_K2 *K2Schema = GetDefault<UEdGraphSchema_K2>();
	UEdGraphPin *Pin = CreatePin(EGPD_Input, K2Schema->PC_String, FName("Selection"), FCreatePinParams());
...
{
	const UEdGraphSchema_K2 *K2Schema = GetDefault<UEdGraphSchema_K2>();
	for (TArray<FName>::TIterator it(Routes); it; ++it)
	{
		CreatePin(EGPD_Output, K2Schema->PC_Exec, *it, FCreatePinParams());
	}
	/*FunctionClass = UUnrealMosquittoRoutingLibrary::StaticClass();
	FunctionName = TEXT("RoutingNotMatched");*/
}
